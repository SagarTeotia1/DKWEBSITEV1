...
function Card({
  ...
  const stackScale = [1, 0.955, 0.91][depth] ?? 0.91;
  const stackY     = [0, 22, 44][depth] ?? 44;
  const bgDim      = [1, 0.6, 0.32][depth] ?? 0.32;
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        opacity: isTop ? cardOpacity : bgDim,
      }}
    ...
    >
      ...
      <div className="relative w-full h-full rounded-[1.75rem] overflow-hidden bg-[#181410] shadow-[0_32px_64px_rgba(0,0,0,0.65)]">
        ...
        {/* Direction badges — top card only */}
        {isTop && (
          <>
            {/* Drag-triggered LOVE badge */}
            <motion.div
              className="absolute top-[33%] right-5 rotate-[14deg]"
              style={{ opacity: watchBadge }}
            >
              <span
                className="block px-3 py-1.5 rounded-lg border-2 border-[#c9a84c] text-[#c9a84c] text-[8px] font-bold tracking-[0.4em] uppercase"
                style={{ fontFamily: "var(--font-dm-sans)" }}
              >
                LOVE ✦
              </span>
            </motion.div>
            {/* Drag-triggered SKIP badge */}
            <motion.div
              className="absolute top-[33%] left-5 -rotate-[14deg]"
              style={{ opacity: skipBadge }}
            >
              <span
                className="block px-3 py-1.5 rounded-lg border-2 border-white/20 text-white/35 text-[8px] font-bold tracking-[0.4em] uppercase"
                style={{ fontFamily: "var(--font-dm-sans)" }}
              >
                SKIP ×
              </span>
            </motion.div>
            {/* Idle resting affordance labels (fade out when dragging) */}
            <motion.div
...
export default function ReelSection() {
  ...
  const [cards, setCards] = useState<number[]>(() => reels.map((_, i) => i));
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-16 lg:px-20 py-20 md:py-28 lg:py-32">
        <div className="flex flex-col lg:flex-row lg:items-center gap-14 lg:gap-20 xl:gap-28">
          ...
          >
            {/* Label */}
            <p
              className="mb-5 inline-flex items-center gap-3 text-[9px] tracking-[0.55em] uppercase text-[#c9a84c]"
              style={{ fontFamily: "var(--font-dm-sans)" }}
            >
            ...
            {/* Headline */}
            <h2
              className="font-serif font-bold text-[#f0ebe0]"
              style={{
                fontFamily: "var(--font-playfair)",
            ...
            {/* Sub-copy */}
            <p
              className="mt-5 text-[0.875rem] leading-[1.9] text-white/30 max-w-[320px] lg:max-w-[280px]"
            ...
            <div className="mt-10">
              ...
              >
                ...
                <span className="text-white/25 group-hover:text-[#c9a84c] group-hover:translate-x-1 transition-all duration-300 text-sm">
                  →
                </span>
              </a>
            </div>
          ...
          {/* ── RIGHT: Card stack + flanking buttons ── */}
          <motion.div
            className="flex-shrink-0 flex items-center justify-center gap-3 lg:gap-5"
          ...
          >
            ...
            >
              ...
              <span
                className="text-[6.5px] tracking-[0.4em] uppercase text-white/20 group-hover:text-white/40 transition-colors"
                style={{ fontFamily: "var(--font-dm-sans)" }}
              >skip</span>
            </button>
            ...
            <div
              style={{
                ...
                height: "clamp(490px, calc(82vw * 1.778 + 44px), 620px)",
                position: "relative",
                flexShrink: 0,
              }}
            >
              <div
                style={{
                  ...
                  left: 0,
                  right: 0,
                  height: "clamp(446px, calc(82vw * 1.778), 576px)",
                }}
              >
                {done ? (
                  <div className="absolute inset-0 rounded-[1.75rem] border border-white/[0.07] flex flex-col items-center justify-center gap-4 text-center px-10">
                    <p
                      className="font-serif italic text-white/28 text-2xl"
                      style={{ fontFamily: "var(--font-playfair)" }}
                    >
                      That&apos;s a wrap.
                    </p>
                    <p
                      className="text-[8px] tracking-[0.5em] uppercase text-white/15"
                      style={{ fontFamily: "var(--font-dm-sans)" }}
                    >
                      You&apos;ve seen it all
                    </p>
            ...
            >
              ...
              <span
                className="text-[6.5px] tracking-[0.4em] uppercase text-[#c9a84c]/30 group-hover:text-[#c9a84c]/60 transition-colors"
                style={{ fontFamily: "var(--font-dm-sans)" }}
              >love</span>
            </button>
          </motion.div>
